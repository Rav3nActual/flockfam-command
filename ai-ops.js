...
function viewAIOps(){
  return `<section class="growth-pilot ai-ops"><div class="page-head"><div><h1>AI Operations</h1><p>A controlled system for using agents to increase FlockFam's capacity without handing them the keys.</p></div><div class="pilot-head-actions"><button class="btn ghost sm" data-pilot-collapse>Close all</button><button class="btn sm" data-pilot-expand>Open all</button></div></div>
    ...
    <div class="panel"><div class="panel-head"><h2>Master-File Architecture <span class="count">the next cleanup—not a rushed rewrite</span></h2></div><div class="pilot-purpose"><strong>The master prompt is now too large to be the only file.</strong><p>Keep a short FlockFam OS pointer at the top, then separate stable context, active playbooks, external-source notes, decisions, and proposed learnings into modular files. Load only the modules needed for the current goal. Preserve one canonical version of every rule and archive superseded material instead of silently deleting history.</p></div><div class="pilot-example"><strong>Recommended structure:</strong> Context (brand, customer, products, operations) · Playbooks (Growth Pilot, creator engine, Field Intel, FLOCK DISPATCH) · Sources (OR-001 onward) · Decisions · Proposed learnings. This should be performed as a deliberate consolidation pass after the PDF review series—not by letting an agent reorganize itself without supervision.</div></div>
  </section>`;
}
