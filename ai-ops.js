...
const AI_OPS_STAGES=[
  ...
  },
  {
    ...
    actions:[
      ...
      "<strong>Shopify Commerce Summary.</strong> Read Shopify's authoritative product, order, customer, and inventory data; summarize the pilot signal; and flag possible stock or checkout issues. It never creates a second inventory ledger or changes the live store.",
  ...
  },
  {
    ...
    actions:[
      ...
      "<strong>Shopify alert packet.</strong> When Shopify shows a stock or commerce exception, prepare the sales pace, cash need, supplier readiness, and recommended next step for approval. Shopify remains the source of truth.",
...
function viewAIOps(){
  return `<section class="growth-pilot ai-ops"><div class="page-head"><div><h1>AI Operations</h1><p>A controlled system for using agents to increase FlockFam's capacity without handing them the keys.</p></div><div class="pilot-head-actions"><button class="btn ghost sm" data-pilot-collapse>Close all</button><button class="btn sm" data-pilot-expand>Open all</button></div></div>
    ...
    <div class="panel"><div class="pilot-purpose"><strong>The useful idea is an operating system—not an AI stunt.</strong><p>Chat answers a question. An agent owns a defined, multi-step result and repeats observe → think → act until the written definition of done is satisfied. For FlockFam, start with reporting, creator research, asset organization, Shopify summaries, store QA, Field Intel, FLOCK DISPATCH preparation, and draft queues. The $10K Growth Pilot strategy and budget stay unchanged.</p></div><div class="pilot-role-note"><strong>FlockFam rule:</strong> Automation earns authority one layer at a time: read → draft → prepare for approval → execute inside a narrow preapproved lane. Access to a tool is access to real authority, so permissions stay minimal and every important action remains reviewable.</div></div>
    <div class="panel"><div class="panel-head"><h2>Current work log <span class="count">simple until volume justifies more</span></h2></div><div class="pilot-purpose"><strong>No separate agent database yet.</strong><p>Material prepared work, approvals, verified results, and failures should be logged as Command Center tasks or linked documents for now. Add a dedicated work-log table only when recurring agent volume makes the shared To-Do genuinely hard to manage.</p></div><div class="pilot-role-note"><button class="btn ghost sm" data-pilot-nav="todo">Open To-Do</button> <button class="btn ghost sm" data-pilot-nav="docs">Open Docs</button></div></div>
    <div class="pilot-stages">${AI_OPS_STAGES.map((stage,index)=>pilotStageCard(stage,index,"ai")).join("")}</div>
    ...
    <div class="panel"><div class="panel-head"><h2>Best First Agents <span class="count">useful to the current business</span></h2></div><div class="pilot-score-wrap"><table class="pilot-score"><thead><tr><th>Agent</th><th>Produces</th><th>May not do</th><th>Human gate</th></tr></thead><tbody>
      ...
      <tr><td>Shopify Commerce Summary</td><td>KPIs and possible stock/checkout exceptions from Shopify</td><td>Create a duplicate inventory record or change the store</td><td>Owner verifies Shopify and approves action</td></tr>
      ...
      <tr><td>FLOCK DISPATCH Assistant</td><td>Captions, hashtags, formatting, proposed times, and complete queue drafts</td><td>Schedule or publish unapproved material</td><td>Owner approves the complete queue item</td></tr>
    </tbody></table></div></div>
